...
class _NonlinearEncoderLayer(torch.nn.Module):
    def __init__(
        self,
        in_features,
        out_features,
...
class LongTermNonlinearEncoder(nn.Module):
    def __init__(self, input_dim, hidden_layer_sizes, output_dim):
        ...
        grid_eps = 0.02
        grid_range = [-1, 1]
        self.kan_layer = _NonlinearEncoderLayer(
    ...
    def forward(self, input_seq):
        ...
        # print(lstm_out.size())  # torch.Size([128, 1, 64])

        output = self.kan_layer(lstm_out[:, -1, :])
        return output
    
    
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):

        return self.kan_layer.regularization_loss(regularize_activation,regularize_entropy)
